...
def start_ddp_trainer(rank, config):
    ...
    store_memory = int(config.ray.store_memory_gb * 1024 * 1024 * 1024)
    ray.init(num_gpus=num_gpus, num_cpus=num_cpus, object_store_memory=store_memory)
    ...
    if rank == 0:
        ...
        if config.ddp.training_size == 1:
            logger_name = 'ocezv2-' + config.env.game + '_seed=' + str(config.env.base_seed)
